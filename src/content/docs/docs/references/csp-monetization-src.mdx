...
The `monetization-src` directive within the HTTP <LinkOut href="https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy" >Content-Security-Policy</LinkOut> (CSP) allows you to restrict the URLs from which a payment pointer can be loaded.
