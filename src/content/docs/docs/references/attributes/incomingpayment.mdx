...
The `incomingPayment` attribute returns the URL that represents the incoming payment at the WM receiver. The returned value is the value it was initialized with. By <LinkOut href="https://openpayments.guide/apis/resource-server/operations/get-incoming-payment/">querying the URL</LinkOut>, you can get the `receivedAmount`.
