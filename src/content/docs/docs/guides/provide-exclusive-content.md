...
If the visitor is web monetized, we'll listen for the `monetization` event that occurs when a payment is sent. When this event fires, the exclusive content will appear and the other `div`s will be hidden.
