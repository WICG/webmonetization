...
The monetization `<link>` element only supports one `href` value. As such, only one payment pointer can be defined within the `<link>`.

[Probabilistic revenue sharing](../set-up-probabilistic-revenue-sharing) is one way to support multiple payment pointers on a page; however, it might not suit your use case. By adding a script to your site, this method will randomly choose from a list of predefined payment pointers each time a visitor loads the page.
