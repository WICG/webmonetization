...
By listening for `monetization` events, you can use the information returned by the following properties to verify payments.

* [amountSent](/experiment/events#amountsent) - The currency amount and code of the sent payment. Contains two variables, `value` and `currency`, constructed by the `MonetizationCurrencyAmount` interface.
* [incomingPayment](/experiment/events#incomingpayment) - A URL you can use to determine if a payment was received and, if so, the amount.
* [paymentPointer](/experiment/events#paymentpointer) - The URL of the wallet address that the incoming payment request was sent to.
