...
:::tip
Wallet providers who support Open Payments issues each of their customers' accounts with an HTTPS address called a [wallet address](#wallet-address). Note that some wallet providers refer to their wallet addresses as payment pointers.
:::
...
A payment pointer is similar to a [wallet address](#wallet-address). Where a wallet address begins with `https://`, a payment pointer begins with `$`. For example, `$wallet.example.com/alice`.

Most payment pointers can be converted to wallet addresses by simply replacing the `$` with `https://`. For example, `https://wallet.example.com/alice`.

:::tip
Whether a wallet provider chooses to call their payment addresses _payment pointers_ or _wallet addresses_ is up to the provider. In places where the documentation only refers to wallet address, it's likely that the same information applies to payment pointers.
:::
...
Not all wallet providers support Web Monetization payments. A provider must implement the Open Payments standard and be able to assign your account a wallet address or payment pointer for you to use the account for Web Monetization.
...
- Parse the wallet address or payment pointer in a monetization `<link>`
...
- Ownership of the financial account's associated wallet address or payment pointer
