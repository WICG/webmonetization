...
- The URL representing the [wallet address](#walletaddress) that an incoming payment request was sent to
...
The `walletAddress` property returns the URL that represents the wallet address that the payment was sent to. In most cases, the value will be the same as the `href` URL in the monetization `<link>`.

:::note[Property naming history]
Earlier versions of the Web Monetization specification only used Interledger's Simple Payment Setup Protocol (SPSP), which calls for payment pointers. This is why the property was originally named `paymentPointer`.

As of the September 2026 spec update, `paymentPointer` is deprecated in favor of `walletAddress`. Both properties return the same value, but `walletAddress` should be used going forward.
:::

#### Example using `walletAddress`
...
<script>
  ...
  link.addEventListener('monetization', (event) => {
    console.log(event.walletAddress)
  });
</script>
```
...
<Specification anchor="walletaddress-attribute" />

### paymentPointer

:::caution[Deprecated]
`paymentPointer` is <LinkOut href="https://developer.mozilla.org/en-US/docs/MDN/Writing_guidelines/Experimental_deprecated_obsolete#deprecated_and_obsolete">deprecated</LinkOut>. Use [`walletAddress`](#walletaddress) instead.
:::

The `paymentPointer` property returns the same URL as [`walletAddress`](#walletaddress). It's retained for backward compatibility only and shouldn't be used in new integrations.
...
You add the vanity address as the `href` value in the monetization `<link>`. Then, you set up an event listener for `walletAddress`. The value that's returned will be:
...
For this use case, let's assume you are familiar with [probabilistic revenue sharing](/tutorials/revenue-sharing), which is itself an advanced use case.
...
You add the generated URL as the `href` value in the monetization `<link>`. Then, you set up an event listener for `walletAddress`. The value that's returned will be which one of the five wallet addresses that the payment was sent to.
